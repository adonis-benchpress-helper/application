...
//     this.assistant = initializeAssistant(() => this.getStateForAssistant());
//     this.assistant.on('data', (event) => this.dispatchAssistantAction(event?.action));
//   }
...
//         switch (action.type) {
//             case 'set_weight':
//                 this.set_weight(action);
//                 break;
//             case 'set_reps':
//                 this.set_reps(action);
//                 break;
//             case 'calculate_max':
...
//         this.setState({ error: 'Вес должен быть в диапазоне от 0 до 300 килограммов' });
//     } else {
//         this.setState({ weight, error: '' });
//     }
//   }
...
//             </CardContent>
//             <Button className={AppCss.countbutton} text="Рассчитать 1ПМ" size="s" view="overlay"
//             onClick={() => this.calculateMaxWeight()} />
//             <Display2 className={AppCss.resmsg}>Ваш одноповторный максимум составляет</Display2>
//             <Display3 className={AppCss.resvalue}>{maxWeight}</Display3>
//             <Display2 className={AppCss.reskg}>килограмм</Display2>
//          </Card>
...
const initializeAssistant = (getState) => {
  ...
  const initPhrase = `Запусти ${process.env.REACT_APP_SMARTAPP}`;
...
export class App extends React.Component {
    constructor(props) {
      ...
      this.assistant.on('start', () => console.log('Assistant started'));
      this.assistant.on('error', (err) => console.error('Assistant error:', err));
    }
